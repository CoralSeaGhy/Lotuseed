...
//    }
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                ...
                (u_int32_t)(arc4random() % 4294967295)];
    }
...
//
+ (NSString*) value {
    return [LSDUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [LSDUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [LSDUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [LSDUDID _setDict:localDict forPasteboard:slotPB];
    }
